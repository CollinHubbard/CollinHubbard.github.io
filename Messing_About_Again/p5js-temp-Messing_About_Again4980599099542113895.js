...
function setup() {
  ...
  currTurn.style("padding", "10px");
  currTurn.style('height', '200px');
  currTurn.style('width', '200px');
  currTurn.style('background-color', team2Color);
  currTurn.style('transform', 'translate(300px)');
...
function checkAllMoves(team) {
  for (let i = 0; i < pieces.length; i++) {
    if (pieces[i].team == team) {

      print(checkValid(pieces[i].x + 200, pieces[i].y + 200, pieces[i]));
      print(checkValid(pieces[i].x - 200, pieces[i].y + 200, pieces[i]));
      print(checkValid(pieces[i].x - 200, pieces[i].y - 200, pieces[i]));
      print(checkValid(pieces[i].x + 200, pieces[i].y - 200, pieces[i]));

      if (checkValid(pieces[i].startX + 200, pieces[i].startY + 200, pieces[i])) {
        return true;
      }
  ...
  return false; // No moves that capture a piece
}
...
class Piece {
  ...
  release() {
    ...
    if (d <= this.d/2) {
      if (this.team == 1) {
        if (this.king == true) {
          this.c = color(team1KingColor);
        } else {
          this.c = color(team1Color);
        }
      } else {
        if (this.king == true) {
          this.c = color(team2KingColor);
        } else {
          this.c = color(team2Color);
        }
      }

...
function setup() {
  ...
  cnv.position((windowWidth - width) / 2, ((windowHeight - height) / 2) - (height / 11));
...
function gameOver(losingTeam) {
  endScreen(losingTeam);
  endGame = true;
}
...
function mousePressed() {
  if (mouseButton === CENTER) {
    gameOver(turn); // Current player loses
  } else {
    for (let i = 0; i < pieces.length; i++) {
      pieces[i].click();
    }
...
function mouseReleased() {
  ...
  if (draggingObj != null) {
    ...
    if (!startEn && checkValid(roundX, roundY, draggingObj) == true) {
